...
exports.ApplicationCustomCredential = exports.ApplicationDefaultCredential = exports.CatalystCredential = exports.CookieCredential = exports.TicketCredential = exports.AccessTokenCredential = exports.RefreshTokenCredential = exports.Credential = exports.globalValue = void 0;
...
const { CREDENTIAL_SUFFIX, REQ_METHOD, CREDENTIAL_HEADER, CREDENTIAL_TYPE, CREDENTIAL_USER, CSRF_TOKEN_NAME, ACCOUNTS_ORIGIN } = utils_1.CONSTANTS;
exports.globalValue = {};
...
function fromEnv() {
    ...
    if (clientId && clientSecret && refreshToken) {
        ...
        throw new errors_1.CatalystAuthError('INVALID_CREDENTIAL', `Failed to get the credential string from env variables`);
    }
}
    ...
    constructor(credObj, scope) {
        ...
        if (this.scope === CREDENTIAL_USER.user) {
            if (this.userType === CREDENTIAL_USER.admin) {
                throw new errors_1.CatalystAuthError('AUTH_ERROR', 'User not authenticated. Please login to proceed with user scope', credObj);
            }
        ...
        switch (this.scope) {
            ...
            case CREDENTIAL_USER.user:
                if (this.userCred === undefined) {
                    throw new errors_1.CatalystAuthError('INVALID_CREDENTIAL', 'User Credential is not initialised', this.scope);
                }
                return this.userCred.getToken();
            default:
                throw new errors_1.CatalystAuthError('INVALID_CREDENTIAL', 'user provided is not recognized', this.scope);
        }
    }
    getScope() {
        return this.scope;
...
class ApplicationCustomCredential extends Credential {
    constructor(credObj) {
        super();
        if (credObj === undefined || credObj === null) {
            throw new errors_1.CatalystAuthError('INVALID_CREDENTIAL', 'Unable to get token object from path or env', credObj);
        }
        if ('refresh_token' in credObj) {
            ...
            throw new errors_1.CatalystAuthError('INVALID_CREDENTIAL', 'The given token object does not contain proper credentials', credObj);
        }
    }
    async getToken() {
        return this.credential.getToken();
...
exports.ApplicationCustomCredential = ApplicationCustomCredential;
